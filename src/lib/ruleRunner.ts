...
import { describeRule, matchRule, type DeclutterRule, type RuleAction } from "./rules";
...
export function previewRuleMatches(rules: DeclutterRule[], senders: SenderSummary[]): number {
  let total = 0;
  for (const rule of rules) {
    if (!rule.enabled) continue;
